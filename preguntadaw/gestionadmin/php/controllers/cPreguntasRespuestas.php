...
require_once __DIR__ . '/cTemas.php';
class cPreguntasRespuestas{
    ...
    public function editarPregunta(){
        ...
        if($pregunta && $respuestas){
            // Para que la ruta de la imagen sea accesible desde el navegador,
            // devolvemos la ruta pública incluyendo el prefijo "php/".
            $rutaPublicaImagen = 'php/' . RUTA_IMAGENES_PREGUNTAS . ($pregunta['imagen'] ?? '');
            echo json_encode ([
                'success' => true,
                'idTema' => $idTema,
                'nPregunta' => $nPregunta,
                ...
                'imagenPregunta' => $rutaPublicaImagen
            ]);
        }else{
            echo json_encode ([
                'success' => false,
                'error' => 'No se ha cargado los datos'
            ]);
        }
    }
    ...
    private function modificarRespuestas($idTema, $nPregunta, $respuestas){
        ...
        header('Location: index.php?controller=Temas&action=listarTemas');
        exit();
    ...
    public function meterPreguntas(){
        ...
        $resultado = $this->modeloPreguntas->meterPreguntasNoSegura($idTema, $nPregunta, $nombreImagen);
        if((int)$resultado > 0){
            // Guardar las respuestas
            if($this->meterPreguntasNoSegura($idTema, $nPregunta)){
    ...
    public function meterRespuestas($idTema, $nPregunta){
        ...
        foreach($respuestas as $indice => $respuesta){
            ...
            $resultado = $this->modeloRespuestas->meterPreguntasNoSegura($idTema, $nPregunta, $letraC, $respuesta, $esCorrecta);
